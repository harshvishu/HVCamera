...
public class Camera21 extends HVBaseFragment {
    private static final String TAG = "Camera21";

    private static final SparseIntArray ORIENTATIONS = new SparseIntArray();
    private static final int REQUEST_CAMERA_PERMISSION = 0x1;
    ...
    private void requestCameraPermission() {
        if (shouldShowRequestPermissionRationale(Manifest.permission.CAMERA)) {
            new RuntimeException("Show a dialog here");
        } else {
    ...
    public void onRequestPermissionsResult(int requestCode, @NonNull String[] permissions,
        ...
        if (requestCode == REQUEST_CAMERA_PERMISSION) {
            if (grantResults.length != 1 || grantResults[0] != PackageManager.PERMISSION_GRANTED) {
                new RuntimeException("Show error dialog");
            }
        } else {
    ...
    private void setUpCameraOutputs(int width, int height) {
        ...
        try {
            for (String cameraId : manager.getCameraIdList()) {
                ...
                System.out.println(":::DEF SENSOR ORIENTATION: " + mSensorOrientation);
